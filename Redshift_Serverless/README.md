...
1. ページ右上にある **ワークグループを作成** をクリックします。
...
1. **名前空間** で ***新しい名前空間を作成** を選択し、**名前空間** に下記のように入力します。**99 の部分はご自分の数字に書き換えて下さい。**
    - `student99-ns`
...
1. クエリエディタ v2 (以降、クエリエディタ）を使用し、テーブルを作成してデータをロード、その後クエリを発行します。これは、下記のドキュメントに記載されている内容を基にしています。
    - https://docs.aws.amazon.com/ja_jp/redshift/latest/gsg/new-user-serverless.html#serverless-load-data-from-s3
  
    - この手順説明では、クエリエディタでライトモードのスクリーンショットを表示しています。ライトモード、ダークモードの切り替えは、クエリエディタの左側のメニューの下側にある三日月のアイコンから行えます。
...
1. **サーバーレスダッシュボード** のページを表示します。

1. **名前空間 / ワークグループ** セクションのリストからご自分が作成したワークグループの名前のリンクをクリックします。
